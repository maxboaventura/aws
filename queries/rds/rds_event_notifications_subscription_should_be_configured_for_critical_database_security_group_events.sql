...
FROM
  ...
  LEFT OUTER JOIN specific_categories ON s.name = specific_categories.source_id
WHERE
  any_category IS NOT TRUE
  AND NOT any_source_categories @> '{"configuration change","failure"}'
  AND (specific_cats IS NULL OR NOT specific_cats @> '{"configuration change","failure"}')
