...
LEFT JOIN aws_applicationautoscaling_policies pr ON pr.namespace='dynamodb' AND pr.resource_id=CONCAT('table/', t.name) AND pr.policy_type='TargetTrackingScaling' AND pr.scalable_dimension='dynamodb:table:ReadCapacityUnits'
LEFT JOIN aws_applicationautoscaling_policies pw ON pw.namespace='dynamodb' AND pw.resource_id=CONCAT('table/', t.name) AND pw.policy_type='TargetTrackingScaling' AND pw.scalable_dimension='dynamodb:table:WriteCapacityUnits'
